...
이 프로젝트에서 성능 개선보다 먼저 해야 했던 일은 모델이 같은 시나리오의 앞뒤를 몰래 보고 있지
않은지 확인하는 것이었다. 입력은 1분 간격, 토양수분·EC·배지온 타깃은 5분 간격이었다. 정리된 자료는
학습 타깃 7,488행과 테스트 타깃 시점 3,456행으로 구성됐다.
...
결론은 모델이 단순해서가 아니었다. 실제 관수량, 배액량, 전날 마지막 함수율, 테스트 시점의 근권 EC와
온도처럼 절대 level을 알려 주는 입력이 없었다. 관측되지 않은 상태를 더 복잡한 보정으로 복원하려는
시도에는 한계가 있었다.
