...
문제 2의 입력은 한 농가의 환경, 제어, 근권, 생육 CSV와 이미지였다. 출력은 진단명 하나가 아니라
상태 진단, 원인 분석, 관리 제안이 포함된 제한 형식의 보고서였다.

모든 파일을 LLM에 넣고 보고서를 쓰게 하는 방식은 간단하지만, 농가와 구역이 섞이거나 원자료에 없는
숫자를 만들거나 이미지의 구역을 잘못 단정할 수 있었다. API 오류가 규칙 답안으로 조용히 대체되면
실제 모델 성능도 알 수 없다.

그래서 최종 시스템을 하나의 LLM이 아니라 입력 조립, 판단, 검증, 렌더링, 감사가 분리된 하네스로
설계했다.
...
이것은 모델을 포기한 결과가 아니다. 자동확정할 근거가 없는 상태를 코드에 정직하게 표현한 선택이다.
멀티모달 LLM 시스템의 신뢰성은 모델 이름보다 무엇을 입력했고, 무엇을 검증했으며, 언제 답변을
거부했는지에 더 크게 좌우됐다.
